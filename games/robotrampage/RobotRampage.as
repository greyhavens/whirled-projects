...
public class RobotRampage extends Sprite
{
    ...
    protected function sendGameState () :void
    {
        ...
        _gameCtrl.sendMessage("update", [baseData, robotData]);
    }
    ...
    {
        var names :Array = _gameCtrl.seating.getPlayerNames();
        ...
        for (var ii :int = 0; ii < count; ii++) {
            // Make new base
            var base :MoonBase = new MoonBase(_gameCtrl, String(names[ii]), ii);
            _bases[ii] = base;
    ...
    public function selectRobot (robot :Robot) :void
    {
        ...
        if (_selection.length == 3) { // FIXME: magic number
            if (Robot.isValidSet(_selection)) {
                ...
                _gameCtrl.sendMessage("setmade" + _myIndex, data, 0);
