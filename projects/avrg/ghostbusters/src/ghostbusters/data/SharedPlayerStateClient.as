...
public class SharedPlayerStateClient
{
    ...
    */
    ...
    {
        if( playerData(playerId, ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_ACTION) !== undefined) {
            return String(playerData(playerId, ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_ACTION));
        }
        else {
            return "none";
        }
    ...
    */
    public static function setIntoRoomProps( player :Player, roomctrl :RoomSubControlServer) :void
    {
        ...
        if (dict[ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_ACTION] != player.action) {
            roomctrl.props.setIn(key, ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_ACTION, player.action);
        }
        
        if (!ArrayUtil.equals( dict[ROOM_PROP_PLAYER_DICT_INDEX_BLOODBONDED], player.bloodbonded )) {
            roomctrl.props.setIn(key, ROOM_PROP_PLAYER_DICT_INDEX_BLOODBONDED, player.bloodbonded);
        }
    }
    ...
    {
        return ClientContext.gameCtrl.room.props.get(ROOM_PROP_PREFIX_PLAYER_DICT + playerId) != null;
    }
