    ...
    implements Hashable
{
...

    {
        ...
        _action = VConstants.AVATAR_STATE_DEFAULT;
...
//            setTime(0);
        ...
        setAction(VConstants.AVATAR_STATE_DEFAULT);
...
//        log.info("}}}");
    ...
    */
    ...
    public function increaseLevel() :void
    {
        ...
        addXP(missingXp);
        ServerContext.server.awardSiresXpEarned(this, missingXp);
    }
    ...
    public function handleMessage (name :String, value :Object) :void
    {
        try{
            ...
            if(name == VConstants.NAMED_EVENT_ADD_XP) {
                ...
                if(action == VConstants.AVATAR_STATE_MOVING_TO_FEED) {
                    log.debug(playerId + " changing to " + VConstants.AVATAR_STATE_FEEDING);
                    actionChange(VConstants.AVATAR_STATE_FEEDING);
                }
...
//                    }
                ...
                if(sire == 0) {
                    ...
                    ServerContext.server.playerInvitedByPlayer(playerId, inviterId);
                }
                else {
                ...
                if(value is RequestStateChangeMsg) {
                    handleRequestActionChange(RequestStateChangeMsg(value));
                }
                else if(value is BloodBondRequestMsg) {
                    handleBloodBondRequest(BloodBondRequestMsg(value));
                }
                else if(value is FeedRequestMsg) {
                    handleFeedRequestMessage(FeedRequestMsg(value));
                }
                else if(value is ShareTokenMsg) {
                    handleShareTokenMessage(ShareTokenMsg(value));
                }
                else {
...
//                    log.debug("  Classname=" + ClassUtil.getClassName(value));
    ...
    protected function handleShareTokenMessage(e :ShareTokenMsg) :void
    {
    ...
    protected function handleFeedRequestMessage(e :FeedRequestMsg) :void
    {
        var game :FeedingRecord;

        if(action == VConstants.AVATAR_STATE_BARED) {
            setAction(VConstants.AVATAR_STATE_DEFAULT);
            return;
        }
        ...
        if(!isVampire()) {
            if(e.targetPlayer == 0) {
                ...
                game = _room.bloodBloomGameManager.requestFeed(
                    e.playerId,
                    -1,
                    false,
                    [0, 0, 0]);//Prey location
            ...
            if(!game.isStarted) {
                ...
                if(_room.isPlayer(e.targetPlayer)) {
                    actionChange(VConstants.AVATAR_STATE_MOVING_TO_FEED);
                }
                else {
...
//        ServerContext.minionHierarchy.updateIntoRoomProps();
    ...
    {
        var eaten :Player = ServerContext.server.getPlayer(targetPlayerId);
        if(eaten == null) {
        ...
        if(eaten.action != VConstants.AVATAR_STATE_BARED) {
            log.warning("feed(" + targetPlayerId + "), eatee is not in mode=" + VConstants.AVATAR_STATE_BARED);
            return;
        }
        ...
        addBlood(bloodEaten);
        ServerContext.server.playerGainedBlood(this, bloodEaten, targetPlayerId);
    ...
    */
    protected function handleBloodBondRequest(e :BloodBondRequestMsg) :void
    {
        var targetPlayer :Player = ServerContext.server.getPlayer(e.targetPlayer);
    ...
    protected function handleRequestActionChange(e :RequestStateChangeMsg) :void
    {
        log.debug("handleRequestActionChange(..), e.action=" + e.state);
        actionChange(e.state);
    }
    ...
    public function actionChange(newAction :String) :void
    {
        ...
        switch(newAction) {
            case VConstants.AVATAR_STATE_BARED:
                ...
                if(_room.bloodBloomGameManager.isPredatorInGame(playerId)) {
                ...
                if(action == VConstants.AVATAR_STATE_BARED) {
                    if(!_room.bloodBloomGameManager.isPreyInGame(playerId)) {
                        setAction(VConstants.AVATAR_STATE_DEFAULT);
                        break;
                    }
                }
            ...
            case VConstants.GAME_MODE_MOVING_TO_FEED_ON_NON_PLAYER:
                ...
                if(!(action == VConstants.GAME_MODE_FEED_FROM_NON_PLAYER ||
                    action == VConstants.AVATAR_STATE_FEEDING)) {

                    setAction(newAction);
                    }

                game = _room.bloodBloomGameManager.getGame(playerId);
                if(game == null) {
                    log.error("actionChange(" + newAction + ") but no game. We should have already registered.");
                    log.error("no game hmmm.  here is the bbmanager:" + _room.bloodBloomGameManager);
                    break;
                }
...
//                }
            ...
            case VConstants.AVATAR_STATE_FEEDING:
            case VConstants.GAME_MODE_FEED_FROM_NON_PLAYER:

                game = _room.bloodBloomGameManager.getGame(playerId);
                if(game == null) {
                    log.error("actionChange(" + newAction + ") but no game. We should have already registered.");
                    log.error("_room._bloodBloomGameManager=" + _room.bloodBloomGameManager);
                    break;
                }
...
//            log.debug("Updating avatar state", "action", action, "avatar.state", avatar.state);
            ...
            if(action == VConstants.AVATAR_STATE_FEEDING ||
...
//        }
    ...
    public function setIntoRoomProps() :void
    {
        try {
            ...
            if (dict[Codes.ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_STATE] != action) {
                log.debug("Setting " + playerId + " action=" + action + " into room props");
                room.ctrl.props.setIn(key, Codes.ROOM_PROP_PLAYER_DICT_INDEX_CURRENT_STATE, action);
            }
        ...
        //Permanent props
            ...
            if(_ctrl.props.get(Codes.PLAYER_PROP_BLOODBONDED) != bloodbonded) {
                ...
                if(_bloodbonded > 0) {//Set the name too
                    var bloodBondedPlayer :Player = ServerContext.server.getPlayer(_bloodbonded);
    ...
    public function setBloodBonded (bloodbonded :int, force :Boolean = false) :void
    {
        ...
        if(oldBloodBond != 0) {//Remove the blood bond from the other player.
            if(ServerContext.server.isPlayer(oldBloodBond)) {
                var oldPartner :Player = ServerContext.server.getPlayer(oldBloodBond);
...
//        if(_feedingRecord.length > VConstants.b
    ...
    public function isVictim() :Boolean
    {
        ...
        if(predator.action == VConstants.AVATAR_STATE_FEEDING && predator.targetId == playerId) {
            return true;
        }
        return false;
    }
    ...
    public function handleAvatarMoved(userIdMoved :int) :void
    {
        ...
        switch(action) {
            ...
            case VConstants.AVATAR_STATE_FEEDING:
                ...
                if(victim != null) {
                    victim.setAction(VConstants.AVATAR_STATE_DEFAULT);
                }
                else {
            ...
            case VConstants.AVATAR_STATE_BARED:
                var predator :Player = ServerContext.server.getPlayer(targetId);
                if(predator != null) {
                    predator.setAction(VConstants.AVATAR_STATE_DEFAULT);
                }
                else {
                    log.error("avatarMoved(), we shoud be breaking off a victim, but there is no victim.");
                }
