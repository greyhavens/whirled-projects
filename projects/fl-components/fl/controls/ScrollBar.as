	...
	public class ScrollBar extends UIComponent {
		...
		protected function thumbPressHandler(event:MouseEvent):void {
			...
			mouseChildren = false; // Should be able to do stage.mouseChildren, but doesn't seem to work.
                        var container :DisplayObjectContainer = thumb;
                        try {
                            while (container != UIComponent.stageAlias) {
                                container = container.parent;
                            }
                        } catch (err :SecurityError) {
                        }
                        _thumbDownComponent = container;

			_thumbDownComponent.addEventListener(MouseEvent.MOUSE_MOVE,handleThumbDrag,false,0,true);
			_thumbDownComponent.addEventListener(MouseEvent.MOUSE_UP,thumbReleaseHandler,false,0,true);
		}
		/**
         * @private (protected)
         *
		...
		protected function thumbReleaseHandler(event:MouseEvent):void {
			...
			_thumbDownComponent.removeEventListener(MouseEvent.MOUSE_MOVE,handleThumbDrag);
			_thumbDownComponent.removeEventListener(MouseEvent.MOUSE_UP,thumbReleaseHandler);
                        _thumbDownComponent = null;
		}
		/**
         * @private (protected)
         *
        ...
        /** Is listened during thumb-down dragging. */
        protected var _thumbDownComponent :DisplayObjectContainer;
	}
}
