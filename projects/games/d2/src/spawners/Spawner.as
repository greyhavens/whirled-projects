...
public class Spawner
{
    ...
    public function Spawner (main :Main, board :Board, game :Game, player :int, loc :Point)
    {
        _main = main;
        _board = board;
        _game = game;
    ...
    public function spawnIfPossible (gameTime :Number, controller :Controller) :void
    {
        ...
        case STATE_SPAWNING:
            ...
            if (gameTime >= _nextSpawnTime) {
                var nextType :String = _toSpawn.shift() as String;
                Assert.isNotNull(nextType);
                
                var critter :Critter = new Critter(
                    _main, _board, _loc.x, _loc.y, nextType, player, _difficulty);
