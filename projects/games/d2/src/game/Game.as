    ...
    implements UnloadListener
{
    ...
    public function Game (main :Main, board :Board, display :Display, controller :Controller)
    {
        _main = main;
        _board = board;
        _display = display;
    ...
    {
        var spawnerClass :Class = _main.isSinglePlayer ? AutoSpawner : PlayerSpawner;
    ...
    {
        if (! _main.isSinglePlayer) { // this is not valid for a single player game
    ...
    {
        _myMoney = allMoney[_main.myIndex];
        _display.updateMoney(_main.myIndex, _myMoney);
    }
