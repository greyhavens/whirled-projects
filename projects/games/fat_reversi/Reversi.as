...
public class Reversi extends Sprite
{
    ...
    protected function setUpPieces () :void
    {
        ...
        graphics.clear();
        graphics.beginFill(0x688C4F);
        graphics.drawRect(0, 0, max, max);
        graphics.endFill();

        graphics.lineStyle(1.2, 0x455C34);
        ...
        if (myTurn && moves.length == 0) {
            ...
            if (_board.getMoves(1 - turnHolder).length == 0) {
                ...
                if (winnerId == 0) {
                    ...
                    _gameCtrl.game.systemMessage(_gameCtrl.getOccupantName(winnerId) + " has won!");
                }

            } else {
                _gameCtrl.game.systemMessage(_gameCtrl.game.getOccupantName(turnHolderId) +
